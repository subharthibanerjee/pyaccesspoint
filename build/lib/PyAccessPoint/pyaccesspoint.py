...
class AccessPoint:
    def __init__(self, channel='6',hwmode='n',wlan='wlan0', inet=None, ip='192.168.45.1', netmask='255.255.255.0', ssid='MyAccessPoint',
                 password='1234567890'):
        self.wlan = wlan
    ...
    def _check_parameters(self):
        ...
        if self.ssid is None:
            ...
            return False
        print('The wifi channel is set up to be {}'.format(self.channel))
        channel = int(self.channel)
        self.channel=str(self.channel)
        if channel not in range(1, 12):
            logging.error('Wi-Fi channel has to be in 1, 11')
            return False
    ...
    def _write_hostapd_config(self):
        with open(self.hostapd_config_path, 'w') as hostapd_config_file:
            hostapd_config_file.write(config.format(self.ssid, self.password, self.wlan, self.channel, self.hwmode))
